...
| 피팅룸 | 옷장에서 아이템을 골라 아바타에 하나씩 입혀보는 룩북. 옷장의 아이템은 **실제 판매 상품의 누끼 사진**이에요 (`tools/build_catalog.py`가 구글 쇼핑에서 찾아 배경을 지운 사진 + 판매처·상품명·가격·링크). 입히면 아바타에는 **체형 골격에 맞춰 그린 옷**이 사진에서 뽑은 대표색(`color`)으로 입혀져요 (사진을 몸 위에 그대로 얹으면 떠 보여서, 사진은 옷장·룩북 카드에서만 보여줘요). 왼쪽 패널에는 입은 상품이 매거진처럼 배치되고, AI 코디 아이템은 ‹ › 로 다른 상품을 넘겨볼 수 있어요. 사진이 없는 아이템은 SVG 옷 그림으로 대신 그려요 |
